...
Many linux distributions have Avahi and OpenSSL already in place, so normally it probably makes sense to choose those options rather than tinysvcmdns or PolarSSL. Libsoxr is available in recent linux distributions and it requires lots of horsepower Ð chances are an embedded processor won't be able to keep up.
...
The `-S` option allows you to specify the kind of "stuffing" or interpolation to be used Ð `basic` (default) for simple insertion/deletion  or `soxr` for smoother resampling-based interpolation.
...
Apart from arguments of Shairport Sync, there are also arguments for controlling the ALSA audio library. ALSA arguments follow a `Ð` on the command line.
...
The `-t` option allows you to specify the type of audio mixer Ð `software` (default) or `hardware`.
...
Basically all it does is put the program in the background ("daemon") mode, selects the default output device and uses software volume control. You should modify it, if possible, to get hardware volume control. Here are some examples of complete commands. If you are modifying the init script, you don't need the `shairport-sync` at the start, but include the `-d` options, as it puts the program into daemon mode. There are some commented-out examples in the init script; see lines 61Ð63.

This give the service a particular name Ð "Joe's Stereo" and specifies that audio device hw:0 be used.
`shairport-sync -d -a "Joe's Stereo" -- -d hw:0`

For best results Ð including getting true mute and instant response to volume control and pause commands Ð you should access the hardware volume controls. Use `amixer` or `alsamixer` or similar to discover the name of the volume controller to be used after the `-c` option.

Here is an example for for a Raspberry Pi using its internal soundcard Ð device hw:0 Ð that drives the headphone jack:
